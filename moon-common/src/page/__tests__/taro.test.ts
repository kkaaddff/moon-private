...
import {remove,writeFile,ensureFile} from  'fs-extra';
import MoonCore from  'moon-core';
import {genTaroPage} from  '../taro';

describe('taro生成页面测试', () => {
  it('商品列表页面生成', async () => {
    let targetDir = join(tmpdir(), 'moon-temp/' ,Math.random()+"");

    await ensureFile(join(targetDir,"src/app.tsx"));
    await writeFile(join(targetDir,"src/app.tsx"),`
     pages: [ 
       //pagePath//
      "pages/entry/index"
    ]
