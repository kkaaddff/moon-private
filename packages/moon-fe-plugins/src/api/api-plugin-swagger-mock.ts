...
export default class SwaggerMock{
  apply(hook) {
    ...
    hook.onResponseSchema.tap("plugin-api-swagger-mock",async (resSchema,context)=>{
      ...
      let mockMethod:IMock = mockFile.mocks.find(item=>item.url===url && item.method===method);
      ...
      if(!mockMethod.data) {
        ...
        try {
          ...
          let  json = await MoonCore.fakeGen.genrateFakeData(
            resSchema,
...
class MockRepos{
  ...
  async save(){
    await Promise.all(Object.values(this.repos).map(mockFile=>{
