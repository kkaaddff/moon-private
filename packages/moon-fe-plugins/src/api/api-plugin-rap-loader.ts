...
export default class RapLoader {
  ...
  loadeWebapiGroup = async () => {
    ...
    if (!model && this.param.url) {
      let content = await loadJson(
        this.param.url ||
          "http://rap2.taobao.org:38080/repository/get?id=253559&excludeProperty=true"
      );
      model = content.data;
    }
    // let content = await loadJson('http://172.23.40.186:8080/repository/get?id=87');

    let webApiGroup = model.modules
      .map((moduleItem) => {
        let apis = filterSameApi(moduleItem.interfaces).map((interfaceItem) => {
          ...
          let responseSchema = {
            ...
            type: "object",
            title: stringJs(interfaceItem.name + "Res")
              .capitalize()
              .camelize(),
            description:
...
function getRequestParam(
  interfaceItem: IInterface,
  type: "request" | "response"
) {
  let requests: IProperty[] = interfaceItem.properties.filter(
    (item) => item.scope === type
  );
  ...
  return root
    .map((item) => findChildRen(item, allProperMap))
    .map((item) => toParam(item, getUrl2Name(interfaceItem.url)));
}

function findChildRen(
  props: IProperty,
  allPropers: { [id: number]: IProperty }
) {
  let children = [];
  ...
  if (proper.type === "Array") {
    ...
    if (!proper.children || proper.children.length === 0) {
      ...
      let properties = proper.children.reduce((acc, next) => {
        acc[next.name] = rap2JsonSchema(
          next,
          preName + firsetUppercase(proper.name)
        );
        return acc;
      }, {});
