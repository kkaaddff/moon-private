 ...
 *
 */
export function cancelCircularRef(
  jsonSchema,
  definitions={},
  superRefs=[],
  pathRefs= {},
...
log('父依赖为:',superRefs,JSON.stringify(jsonSchema,null,2));
  ...
  for (let _key in jsonSchema) {
    ...
    if (jsonSchema[_key] && jsonSchema[_key].$ref) {
      ...
      if(allPathRefs.includes(jsonSchema[_key].originalRef)) {//如果这条
       console.log(`断开循环引用: 当前类型:${jsonSchema[_key].originalRef},父路径已经引用类型${allPathRefs}`,);
       jsonSchema[_key] = {
          "type": "null",
          "default": null,
        }
        ...
        cancelCircularRef( definitions[jsonSchema[_key].originalRef], definitions,pathRefs[_key].concat(superRefs));
      }
    }
